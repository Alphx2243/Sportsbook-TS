...
import { getGymBookings } from '@/actions/gym';
          ...
          {bookings.length > 0 ? (
            <div className="grid grid-cols-1 gap-6">
              {bookings.map((booking, idx) => (
                <GymCard key={booking.id || idx} gymBooking={booking} index={idx} />
              ))}
            </div>
          ) : (
            <motion.div
          ...
          <div className="md:border-l border-white/5 md:pl-8 flex flex-col justify-center min-w-[150px]">
            ...
            <div className="flex flex-wrap gap-2">
              {booking.issuedEquipments && booking.issuedEquipments.length > 0 ? (
                booking.issuedEquipments.map((item: string, i: number) => {
    ...
    >
      ...
      <div className="p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border ${gymBooking.status === 'active'
                ? 'bg-green-500/10 text-green-500 border-green-500/20 animate-pulse' : 'bg-white/5 text-gray-500 border-white/5'
                }`}>
                {gymBooking.status}
              </span>
            </div>
            <div className="flex flex-wrap items-center gap-4 mt-2">
              <div className="flex items-center gap-1.5 text-gray-400 text-sm">
                <Calendar className="w-4 h-4" />
                <span>{ParseDate(gymBooking.entryTime)}</span>
              </div>
              <div className="flex items-center gap-1.5 text-gray-400 text-sm">
                <Clock className="w-4 h-4" />
            ...
            <div className="flex flex-wrap items-center gap-4 mt-2">
              <div className="flex items-center gap-1.5 text-gray-400 text-sm">
                <Calendar className="w-4 h-4" />
                <span>{ParseDate(gymBooking.exitTime)}</span>
              </div>
              <div className="flex items-center gap-1.5 text-gray-400 text-sm">
                <Clock className="w-4 h-4" />
