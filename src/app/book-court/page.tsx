...
export default function AgniCodersBooking() {
  ...
  const { user, loading } = useAuth() as unknown as { user: User | null; loading: boolean };
  const [sportsData, setSportsData] = useState<Sport[]>([]);
  const [sportsList, setSportsList] = useState<string[]>([]);
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [numPlayers, setNumPlayers] = useState(1);
  const [initialInUse, setInitialInUse] = useState<Record<string, number>>({});
  ...
  useSocket(sportsList[selectedIndex], () => {
    refreshSports(true);
  });
  ...
  const initSport = (sport: Sport, idx: number) => {
    ...
    const courtArr =
      cData.length
        ? cData.map((item: any) => item.split(":")[0])
  ...
  const availableCourts = isCapacityBased
    ? ((selectedSportData.maxCapacity || 0) - (selectedSportData.numPlayers || 0))
          ...
          {!isCapacityBased && (
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                {Array.from({ length: sportsData[selectedIndex]?.numberOfCourts || 0 }, (_, i) => sportsData[selectedIndex]?.courtData?.[i] || `Court${i + 1}:0`).map((item: string) => {
