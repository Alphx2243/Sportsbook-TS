...
import { deleteApplication, getApplications } from '@/actions/applications';
import { deleteInvite, editInvite, getInvites } from '@/actions/invites';
...
const Post = ({ id, mobilenumber, name, time, sportname, venue, date, onDelete, email }: PostProps) => {
...
interface ApplicationCardProps {
  id: string;
  option: 'mentor' | 'assistance';
  sportName: string;
  level: string;
  description: string;
  time: string;
  avDays: string[] | string;
  email: string;
  onDelete: (id: string) => void;
}

const ApplicationCard = ({ id, option, sportName, level, description, time, avDays, email, onDelete }: ApplicationCardProps) => {
  const { user } = useAuth() as { user: any }
...
const CurrentInvites = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setInvites(invitesData.success ? invitesData.data : [])
        setApplications(appsData.success ? appsData.data : [])
      } catch (err) {
        console.error(err)
      }
    }
  ...
  const handleDeleteInvite = async (id: string, status: string) => {
    try {
      let res;
      if (status === 'completed') {
        res = await editInvite(id);
      }
      else {
        res = await deleteInvite(id);
      }
      if (res.success) {
